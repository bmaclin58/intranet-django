...
def preview(request, type_id, model_admin) :
	...
	return render(
			request, 'helpdesk/create.html',
			{'form' : unbound.bind(request = request), 'kind' : kind, 'preview' : True, 'nav' : 'new'},
			)
...
def reports(request, model_admin) :
	...
	totals = tickets.aggregate(
			total = Count('pk'),
			open = Count('pk', filter = Q(status__in = Ticket.OPEN_STATUSES)),
	...
	context = {
			**model_admin.admin_site.each_context(request),
			...
			'filters'    : filters,
			'totals'     : totals, 'by_category' : by_category,
			'by_status'  : by_status,
			'page'       : Paginator(tickets, 25).get_page(request.GET.get('page'),),
			'opts'       : Ticket._meta,
			'report_url' : reverse('admin:helpdesk_ticket_reports'),
			}
