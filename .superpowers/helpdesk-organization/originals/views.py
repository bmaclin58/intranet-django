...
from .forms import ReportFilters, reply_form, snapshot_answers, ticket_form
...
def index(request):
    ...
    return render(request, 'helpdesk/index.html', {
        'page': Paginator(tickets, 20).get_page(request.GET.get('page')), 'query': query,
...
def create(request, type_id):
    kind = get_object_or_404(TicketType.objects.select_related('category'), pk=type_id, active=True, category__active=True)

    def submit(form, **_):
        if not form.is_valid():
            return None
        ticket = Ticket(category=kind.category, ticket_type=kind, requester=request.user,
                        subject=form.fields.subject.value, description=form.fields.description.value,
                        priority=form.fields.priority.value, answers=snapshot_answers(form, definitions))
        try:
    ...
    if response is not None:
        return response
    return render(request, 'helpdesk/create.html', {'form': form, 'kind': kind, 'nav': 'new'})
...
def detail(request, pk):
    ...
    can_reply = is_owner or (can_handle_tickets(request.user) and request.user.has_perms(['helpdesk.change_ticket', 'helpdesk.add_ticketcomment']))
    ...
    def submit(form, **_):
        ...
        if not form.is_valid():
            return None
        comment = TicketComment(ticket=ticket, author=request.user, body=form.fields.body.value or '', internal=False)
        if form.fields.attachments.value and not is_owner and not request.user.has_perm('helpdesk.add_ticketattachment'):
            raise PermissionDenied
        try:
...
def attachment(request, pk):
    item = get_object_or_404(TicketAttachment.objects.select_related('comment', 'ticket'), pk=pk, ticket__in=visible_tickets(request.user))
    if item.ticket.requester_id != request.user.pk and not request.user.has_perm('helpdesk.view_ticketattachment'):
        raise Http404
    if item.comment_id and item.comment.internal and not (can_handle_tickets(request.user) and request.user.has_perm('helpdesk.view_ticketcomment')):
        raise Http404
    try:
        file = item.file.open('rb')
    except (OSError, ValueError) as exc:
        raise Http404('Attachment unavailable.') from exc
    response = FileResponse(file, as_attachment=True, filename=item.original_name, content_type='application/octet-stream')
